...
**Colour and Leadbeater's key.** For six emotions — fear, sadness, love, pride, envy, and compassion — the atlas follows the colour key of C. W. Leadbeater's *Man Visible and Invisible* (1902, ch. XIII and Plate I), a Theosophical account of the human aura in which each emotion is given a characteristic hue: fear is "livid grey", depression "heavy leaden grey", selfless love rose, pride orange, jealousy greenish-brown, and sympathy and compassion "a lovely pale, luminous blue-green". Leadbeater presented these colours as clairvoyant observation, and nothing here depends on that claim; the key is used as a coherent, historically influential visual vocabulary for feeling — the same key, published with Besant in *Thought-Forms* (Besant & Leadbeater, 1901), is part of the Theosophical colour language that shaped early abstract painting (Ringbom, 1966). It agrees with modern association data in places (sadness with grey) and departs from it in others (fear, which people more often associate with black; Jonauskaite et al., 2020). Leadbeater also describes feelings by *form* as well as hue — anger as flashes, hatred as clouds, depression as parallel bars like a cage — and treats every visible colour as a mixture; the atlas records only the hue.
...
| Multisensory signature | Each feeling carries a colour, tone, mode, rhythm, and geometry; brain rhythm feeds back into the feeling (§4.4) | Implemented; expressed through voice, face, and dashboard, not described to the model; only the rhythm mapping feeds back (§4.4) |
